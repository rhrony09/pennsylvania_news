<div {{ $attributes->merge(['class' => 'mt-3']) }}>
    ...
    <input type="text" class="form-control" placeholder="{{ isset($fieldPlaceholder) ? $fieldPlaceholder : '' }}" value="{{ isset($fieldValue) ? $fieldValue : '' }}" name="{{ $fieldName }}" id="{{ $fieldId }}" @isset($fieldRequired) required @endisset autocomplete="off">
    @isset($fieldHelp)
