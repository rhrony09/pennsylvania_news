<div {{ $attributes->merge(['class' => 'mt-3']) }}>
    ...
    <input type="text" class="form-control" placeholder="{{ $fieldPlaceholder }}" value="{{ $fieldValue }}" name="{{ $fieldName }}" id="{{ $fieldId }}" @isset($fieldRequired) required @endisset @isset($fieldReadOnly) readonly @endisset>
    @isset($fieldHelp)
